...
class PlaylistShow extends React.Component {
  ...
  handlePlay(e) {
    ...
    this.props.receiveCurrentPlaylist(
      this.props.playlists[this.props.playlistId]
    );
  }
  ...
  componentDidMount() {
    this.props
      .requestPlaylists()
      .then(res => this.props.requestPlaylist(this.props.playlistId));
  }

  handleFollow(e) {
    this.props.createPlaylistFollow({
  ...
  handleUnfollow(e) {
    this.props.deletePlaylistFollow({
      user_id: this.props.currentUser,
      playlist_id: this.props.playlistId
    });
  }
  ...
  removable() {
    let playlist = this.props.playlists[this.props.playlistId] || {
      user_id: ""
    };
  ...
  render() {
    ...
    allsongs = songs.map(song => {
      return (
        <div className="song-listing-div">
          <SongContainer
            key={song.id}
      ...
      <div className="playlist-show">
        <div className="playlist-show-details-container">
          <div className="playlist-show-cover">
            <div className="show-musical-note">
              <i className="fa fa-music" aria-hidden="true" />
            </div>
          </div>
          <div className="playlist-show-details-text">
            <h6 className="playlist-show-title">{playlist.name}</h6>
            <h6 className="playlist-detail-text-h6">
              By {playlist.user.username}
            </h6>
            <h6 className="playlist-detail-text-h6">
              {songCount} {countWord}
            </h6>
            <button className="playlist-play" onClick={this.handlePlay}>
              PLAY
            </button>
